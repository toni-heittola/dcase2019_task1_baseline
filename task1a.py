...
def main(argv):
    ...
    # Log title
    log.title('DCASE2019 / Task1A -- Acoustic scene classification')
    log.line()
...
def do_evaluation(db, folds, param, log, application_mode='default'):
    ...
    class_wise_results = numpy.zeros((3, len(db.scene_labels())))
    fold = 1
    ...
    reference_scene_list_seen = dcase_util.containers.MetaDataContainer()
    reference_scene_list_unseen = dcase_util.containers.MetaDataContainer()

    for item_id, item in enumerate(reference_scene_list):
        city = item.identifier.split('-')[0]
        reference_scene_list[item_id]['filename'] = os.path.split(item.filename)[-1]
        reference_scene_list[item_id]['file'] = item.filename

        if city in cities['seen']:
            reference_scene_list_seen.append(item)
        if city in cities['unseen']:
            reference_scene_list_unseen.append(item)
    ...
    estimated_scene_list_seen = dcase_util.containers.MetaDataContainer()
    estimated_scene_list_unseen = dcase_util.containers.MetaDataContainer()
    for item_id, item in enumerate(estimated_scene_list):
        city = os.path.split(item.filename)[-1].split('-')[1]
        estimated_scene_list[item_id]['filename'] = os.path.split(item.filename)[-1]
        estimated_scene_list[item_id]['file'] = item.filename

        if city in cities['seen']:
            estimated_scene_list_seen.append(item)
        if city in cities['unseen']:
            estimated_scene_list_unseen.append(item)
    ...
    evaluator_seen = sed_eval.scene.SceneClassificationMetrics(
        scene_labels=db.scene_labels()
    )

    evaluator_seen.evaluate(
        reference_scene_list=reference_scene_list_seen,
        estimated_scene_list=estimated_scene_list_seen
    )

    results_seen = dcase_util.containers.DictContainer(evaluator_seen.results())
    all_results_seen.append(results_seen)

    evaluator_unseen = sed_eval.scene.SceneClassificationMetrics(
        scene_labels=db.scene_labels()
    )

    evaluator_unseen.evaluate(
        reference_scene_list=reference_scene_list_unseen,
        estimated_scene_list=estimated_scene_list_unseen
    )

    results_unseen = dcase_util.containers.DictContainer(evaluator_unseen.results())
    all_results_unseen.append(results_unseen)
    ...
    for scene_label_id, scene_label in enumerate(db.scene_labels()):
        ...
        class_wise_results[1, scene_label_id] = results_seen.get_path(
            ['class_wise', scene_label, 'accuracy', 'accuracy']
        )

        class_wise_results[2, scene_label_id] = results_unseen.get_path(
            ['class_wise', scene_label, 'accuracy', 'accuracy']
        )

    overall = [
        results.get_path('class_wise_average.accuracy.accuracy'),
        results_seen.get_path('class_wise_average.accuracy.accuracy'),
        results_unseen.get_path('class_wise_average.accuracy.accuracy'),
    ]
    ...
    # Table header
    column_headers = ['Scene', 'Accuracy', 'Seen cities', 'Unseen cities']
    column_widths = [20, 25, 20, 20]
    column_types = ['str20', 'float1_percentage', 'float1_percentage', 'float1_percentage']
    column_separators = [True, True, True]
