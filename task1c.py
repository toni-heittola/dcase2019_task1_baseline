...
# -*- coding: utf-8 -*-
# DCASE 2019
# Task 1C: Acoustic Scene Classification
# Baseline system
# ---------------------------------------------
...
from task1a import do_feature_extraction, do_feature_normalization
...
def main(argv):
    ...
    args, overwrite = handle_application_arguments(
        param=param,
        application_title='Task 1C: Acoustic Scene Classification',
        version=__version__
    )
    ...
    # Log title
    log.title('DCASE2019 / Task1C -- Acoustic scene classification')
    log.line()
        ...
        if application_mode == 'dev':
            ...
            if db_eval.reference_data_present and param.get_path('flow.evaluation'):
                if application_mode == 'eval':
                    ...
                    do_evaluation_task1c_leaderboard(
                        db=db_eval,
                        folds=active_folds,
                        param=param,
                        log=log,
...
def do_learning(db, folds, param, log, overwrite=False):
    ...
    for fold in folds:
        ...
        if not os.path.isfile(fold_model_filename) or overwrite:
            ...
            # Create list of known scene classes
            scene_labels = db.scene_labels()
            if 'unknown' in scene_labels:
                del scene_labels[scene_labels.index('unknown')]
            ...
            for chain in param.get_path('meta_processing_chain.parameters.chain'):
                ...
                if processor_name == 'dcase_util.processors.OneHotEncodingProcessor':
                    init_parameters['label_list'] = scene_labels
                    init_parameters['allow_unknown_labels'] = True
            ...
            # Train only with known classes
            training_meta = db.train(fold) #db.train(fold).filter(scene_list=scene_labels)
            ...
            model_parameter_constants = {
                'CLASS_COUNT': int(len(scene_labels)), #int(db.scene_label_count()),
    ...
    for fold in folds:
        ...
        if not os.path.isfile(fold_results_filename) or overwrite:
            ...
            for item in db.test(fold=fold):
                ...
                elif numpy.sum(frame_decisions[:, 0]) > 1:
                    frame_decisions = dcase_util.data.ProbabilityEncoder().binarization(
                        probabilities=probabilities,
...
def do_evaluation(db, folds, param, log, application_mode='default'):
    ...
    # Extra information about the unknown classes
    meta_unknown = dcase_util.containers.ListDictContainer(filename=os.path.join(db.local_path, 'meta_unknown.csv')).load()
    unknown_class_to_file_map = {}
    for item in meta_unknown:
        if item['scene_label_original'] not in unknown_class_to_file_map:
            unknown_class_to_file_map[item['scene_label_original']] = []
        unknown_class_to_file_map[item['scene_label_original']].append(os.path.split(item['filename'])[-1])

    # Create list of known scene classes
    known_scene_labels = db.scene_labels()
    if 'unknown' in known_scene_labels:
        del known_scene_labels[known_scene_labels.index('unknown')]
    unknown_scene_labels = ['unknown']
    ...
    unknown_detailed_results = {}

    for hidden_scene_labels in list(unknown_class_to_file_map.keys()):
        current_files = unknown_class_to_file_map[hidden_scene_labels]
        current_reference = reference_scene_list.filter(file_list=current_files)

        if len(current_reference):
            evaluator = sed_eval.scene.SceneClassificationMetrics(
                scene_labels=db.scene_labels()
            )
            evaluator.evaluate(
                reference_scene_list=current_reference,
                estimated_scene_list=estimated_scene_list.filter(file_list=current_files)
            )
            current_results = dcase_util.containers.DictContainer(evaluator.results())
            unknown_detailed_results[hidden_scene_labels] = current_results.get_path('overall.accuracy') * 100.0
    ...
    # Class-wise rows
    accuracy_known_scenes = []
    for scene_label in known_scene_labels:
        scene_label_id = db.scene_labels().index(scene_label)
        log.row(
            scene_label,
            class_wise_results[0, scene_label_id] * 100.0
        )
        accuracy_known_scenes.append(class_wise_results[0, scene_label_id])

    log.row_sep()

    column_values = ['CLASS AVG']
    column_values += [
        numpy.mean(accuracy_known_scenes) * 100.0
    ]
    ...
    log.row_sep()
    accuracy_unknown_scenes = []
    for scene_label in unknown_scene_labels:
        scene_label_id = db.scene_labels().index(scene_label)
        log.row(
            scene_label,
            class_wise_results[0, scene_label_id] * 100.0
        )
        accuracy_unknown_scenes.append(class_wise_results[0, scene_label_id])

    log.row_sep()
    log.line('Unknown scene classes')
    for scene_label in list(unknown_detailed_results.keys()):
        log.row(
            scene_label,
            unknown_detailed_results[scene_label]
        )

    log.line()
    log.row_sep()

    column_values = ['ACC (CLASS_AVG|UNK)']
    rank_score = 0.5*numpy.mean(accuracy_known_scenes) + 0.5*numpy.mean(accuracy_unknown_scenes)
    column_values += [rank_score * 100.0]
